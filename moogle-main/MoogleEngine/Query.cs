...
public class Query
{
            ...
            switch (currenttoken)
            {
                case token.Importancia: // encontrado un *
                    if (
                        ...
                        || (beforetoken == token.Ninguno)
                        || (beforetoken == token.Importancia)
                        || ((beforetoken == token.Cercania) && (beforeword != ""))
                    )
                    {
                        contimp++;
                        beforetoken = token.Importancia;
                ...
                case token.Palabra: //se encontro una Palabra
                    ...
                    if (beforetoken == token.Cercania) // anterior a la palabra un ~ siguiendo a beforeword
                    {
                        FillQueryDictionary(
                            ...
                            0,
                            wordalreadyprocess
                        ); // Se procesa operador de cercania para las los palabras. wordalreadyprocess = false
                        cercaniapendiente = false; // Cercania ya procesada
                    }
                    else if (beforetoken == token.Importancia) // Operador * delante de la palabra
                    {
                        ...
                        FillQueryDictionary(
                            ...
                            contimp,
                            wordalreadyprocess
                        ); // Se procesa Operador * para currentword. wordalreadyprocess= true
                        contimp = 0;
                    }
                    else if (beforetoken == token.Omitir) // Operador ! delante de la palabra
                    ...
                    else if (beforetoken == token.Obligacion) // Operador ^ delante de la palabra
                    {
                        ...
                        if (contimp != 0) // La palabra tenia * y ^
                        {
                            FillQueryDictionary(
                                ...
                                contimp,
                                wordalreadyprocess
                            ); // Se procesan los operadores * y ^ para currentword.
                            contimp = 0;
                        }
                        else
                            FillQueryDictionary(
                                ...
                                0,
                                wordalreadyprocess
                            ); // Se procesa el operador ^ para currentword.
                    }
                    else if ((beforetoken == token.Ninguno) || (beforetoken == token.Palabra))
    ...
    private token NextToken(string query, out string word)
    {
        ...
        while (pos < query.Length)
        {
            ...
            else if (Char.IsLetterOrDigit(query[pos])) // Inicio de Palabra
            {
    ...
    private void FillQueryDictionary(string word1, string word2, token tk, int contimp, Boolean wap)
    {
        ...
        {
            if (!wap)
                querydictionary[word1][0]++; // La palabra ya existe en el Dicc., pero en otra Posici{on dentro de la Query, No se habia procesado antes. Se cuenta nuevamente.
        }
        else
        {
        ...
        switch (tk)
        {
            ...
            case token.Cercania:
                if (cercanas.ContainsKey(word1)) // word1 ya esta en el Diccionario como Key
                {
                    ...
                    cercanas.Add(word1, new List<string> { word2 }); // Si la palabra no existe en el Dic, se agrega como Key y se agrega word2 a la lista
                break;
        }
    }
