...
///</summary>
    public Corpus(string path,string fileext)//, string inputquery)
    {
        ...
        if (Directory.Exists(path))                        // Verifica si el directorio existe
        {
         ...
         if (files.Length == 0) error=2;         // Directorio vacio
         else
         {
...
///</summary>
    public void FillTFIDFMatrix()
    {
        ...
        for (int i = 0; i < docs.Length; i++)
        {
            ...
            {    
                tfidfmatrix [i, index++] = GetTF(docs[i],aux) * (float)Math.Log10((float)docs.Length / (float)vocabulary[aux]);
            }
        }
         ...
         for (int i=0; i < wordlist.Count; i++)          // Recorre la lista de palabras cercanas a word1
            {
              ...
              if (docs[docindex].Docsdictionary.ContainsKey(word1) && docs[docindex].Docsdictionary.ContainsKey(word2))    // Las dos palabras con ~, estan en el Documento. 
               { 
                  nearaux = DistanciaMinima(word1, word2, docindex);
                  if (nearaux == 0)     
                   cnearsnotindoc++;                      // las palabras cercanas son iguales y aparece una sola vez 
                  else 
                  if (nearaux < near)                  // La distancia encontrada es menor que la que existia
                   {
                     near = nearaux;                 // en near queda la distancia minima entre todos los pares de palabras que estan en el Documento
                     cnearsindoc++;
                   }
               }
             else cnearsnotindoc++;                       // Al menos una de las palabras no esta en el Documento, Se cuenta y se continua  
            }
      }
     
     if (cnearsindoc > cnearsnotindoc) return near;       // El Documento contiene mayoria de las palabras enlazadas y se retorna la minima distancia entre ellas
     else  return docs[docindex].Wordcount;               // El Documento contiene la minoria de las palabras, sera penalizado su score.
   }
...
///</summary>
    public float GetScore(int docnumber)         
    {
        ...
        if (deno != 0f)  similaridadcoseno = (float)similaridadprodescalar / deno;
    ...
    public string FindSuggestion()                   
    {
        ...
        foreach (var wordquery in query.Querydictionary.Keys)       // para cada palabra de la query
        {
            ...
            {
               if (syn is null) syn = new Synonymous();     // Si no se ha creado el objeto syn, se instancia la clase Synonymous
                 ...
                 if (suggestion == "")  // Ningun Sinonimo esta en los Documentos del Corpus. Se aplica Levenshtein Para buscar Similitud entre los Sinonimos y las palabras del Corpus    
                  {  
                   foreach (string synword in auxsyn) 
                    ...
                    if (suggestion != "") break;            //Salgo del Ciclo synword
                    }
                  }
               }
            }
    ...
    private float LevenshteinSimilarity(string wordvoc, string wordquery)                        
    {
        ...
        if ((n == 0) || (m == 0)) return 0f;   // si una de las cadenas es vacia => Similitud 0
